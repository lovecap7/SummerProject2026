...
PS_OUTPUT main( PS_INPUT PSInput )
{
	...
	//「このピクセルの面が、ライトの方向をどれくらい正面から向いているか」を0～1の数値にしている
    float lLightDot = saturate(dot(Normal, -lLightDir));

	// 3段階に区切る例
    if (lLightDot > 0.7f)	//ほぼ正面
        DiffuseAngleGen = 2.0f;
    else if (lLightDot > 0.3f)	//ほぼ上
        DiffuseAngleGen = 1.0f;
    else	//90度以上
        DiffuseAngleGen = 0.2f;
