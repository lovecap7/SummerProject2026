...
// アウトライン描画を行う場合は下記を有効化(通常描画用と分けてコンパイルする場合はコンパイルオプション側でdefineしてもOK)
#define OUTLINE 1

#include "VertexShader.h"

#ifdef OUTLINE
// アウトラインの太さ・色等のパラメータ(空いているスロット番号に合わせて調整してください)
cbuffer OUTLINE_PARAM : register( b7 )
{
	float  g_OutlineWidth ;		// アウトラインの太さ(ビュー空間でのオフセット量の基準値)
	float3 g_OutlineReserve ;	// 予約領域(16byteアライメント調整用)
} ;
#endif // OUTLINE
...
VS_OUTPUT main(VS_INPUT VSInput)
{
	...
	// 法線を計算( ワールド→ビュー )
	// ※アウトライン処理で先に法線(ビュー空間)が必要になるため、通常版より前段に移動
    ...
    lViewPosition.w = 1.0f;
		...
		//アウトライン用: ビュー空間の法線方向へ頂点を押し出す
		//カメラからの距離( lViewPosition.z )に応じて太さを補正し、遠近感による太さの破綻を軽減
		float lOutlineScale = abs( lViewPosition.z ) * 0.01f ;
		lViewPosition.xyz += normalize( lViewNrm ) * g_OutlineWidth * lOutlineScale ;
