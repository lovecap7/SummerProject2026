...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    int bloomH = LoadPixelShader(L"Bloom.pso");
    assert(bloomH != -1);

    int modelH = MV1LoadModel(L"Human.mv1");
    int model2H = MV1DuplicateModel(modelH);
    int model3H = MV1DuplicateModel(modelH);
    assert(modelH != -1);
    ...
    int rt3 = MakeScreen(width, height, true);
    ...
    while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
    {
        ...
        MV1SetRotationYUseDir(model2H, VGet(0, 1, 0), angle);
        MV1SetRotationYUseDir(model3H, VGet(0, 1, 0), angle);
        MV1SetPosition(model2H, VGet(-100, 0, 100));
        MV1SetPosition(model3H, VGet(100, 0, 100));

        // 毎フレーム
        animTime += 1.0f;

        MV1SetAttachAnimTime(modelH, animIndex, animTime);
        MV1SetAttachAnimTime(model2H, animIndex, animTime);
        MV1SetAttachAnimTime(model3H, animIndex, animTime);
        ...
        // 1パス目: アウトライン(背面のみ描画、法線方向に膨らませ、ゲーミング色)
        SetUseVertexShader(outLineVSH);
        SetUsePixelShader(outLinePSH);
        MV1DrawModel(modelH);
        MV1DrawModel(model2H);
        MV1DrawModel(model3H);
        ...
        MyDraw::DrawImageHandle(0, 0, rt3, bloomH,4);
